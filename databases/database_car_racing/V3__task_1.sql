...
MinAvgPosition AS (
    ...
    FROM CarResults  -- группируем данные по каждому классу, сортируем автомобили внутри каждого класса по средней позиции и RANK присваиваем ранги автомобилям с наименьшей средней позицией
)

SELECT car AS car_name, class AS car_class, avg_pos AS average_position, race_count
FROM MinAvgPosition
WHERE rank = 1   -- выборка втомобилей только с рангом 1
ORDER BY avg_pos;  
