WITH CarResults AS (
    SELECT c.name AS car_name, c.class AS car_class, AVG(r.position) AS average_position,
        ...
        HAVING average_position > 3.0  -- выбрать автомобтили с "низкой" средней позицией
),

ClassResults AS (   -- узнаем количество автомобилей и общее количество гонок для каждлого класса
