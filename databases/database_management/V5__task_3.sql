-- рекурсивный СТЕ для поиска всех менеджеров и их подчинённых

WITH RECURSIVE ManagerHierarchy AS (
    -- базовый уровень: выбираем всех сотрудников, у кого роль "Менеджер"
    ...
    -- рекурсивный уровень: находим всех подчинённх сотрудников каждого менеджера
    SELECT e.EmployeeID, e.Name, e.ManagerID, e.DepartmentID, e.RoleID
    FROM Employees e
    INNER JOIN ManagerHierarchy mh ON e.ManagerID = mh.EmployeeID
)

-- основной запрос для поиска информации и их подчинённых
...
HAVING TotalSubordinates > 0  --выбираем только тех сотрудников, у кого есть подчинённые
ORDER BY mh.Name;
